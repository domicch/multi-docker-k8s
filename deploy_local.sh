...
kubectl apply -f k8s/project_local
...
kubectl set image deployment/server-deployment server=domicch/multi_docker_k8s_server:latest
kubectl set image deployment/client-deployment client=domicch/multi_docker_k8s_client:latest
kubectl set image deployment/worker-deployment worker=domicch/multi_docker_k8s_worker:latest
